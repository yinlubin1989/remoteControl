...
固件同时通过手柄 `axes[0]` 和 `axes[3]` 上报 `500–2500 μs` 的 CH1/CH2 原始脉宽；网页连接 `RC Car Controller` 后会显示并直接使用这两路 PWM 控车。网页默认按 `1000/1500/2000 μs` 三点转换控制量，也可在状态栏分别采样方向和油门的实际两端与中位；任一通道信号丢失时两路立即回中。

固件内部校准后的标准控制轴仍保留在 `axes[1]` 和 `axes[2]`，供其他 Gamepad API 使用方使用；本项目网页不依赖这两个轴的校准/回中解锁状态。

网页状态栏中的方向、油门当前 PWM 均可点击，分别打开遥控器输入三点校准。方向可设置左行程、中位、右行程，油门可设置前进行程、中位、后退行程；网页按两侧的实际行程分段换算控制比例，并支持数值反向的通道。该校准只保存在当前浏览器，不改变车辆输出中位：方向仍使用控制端设置值，油门仍固定为 `1500 μs`。校准弹窗打开时车辆保持中立，关闭后需要方向和油门连续回中 `500 ms` 才会重新接管。
